...
pub const MemoryStorage = struct {
    ...
    pub fn loadTerm(ptr: *MemoryStorage) Term {
    ...
    pub fn loadVotedFor(ptr: *MemoryStorage) ?ServerId {
    ...
    pub fn loadLogEntry(ptr: *MemoryStorage, index: LogIndex, allocator: std.mem.Allocator) ?LogEntryOwned {
        // Linear scan (acceptable for tests)
    ...
    pub fn appendLogEntry(ptr: *MemoryStorage, entry: LogEntryOwned) !void {
        ...
        try ptr.entries.append(ptr.allocator, LogEntryOwned{
            .term = entry.term,
            .index = entry.index,
            .data = data_copy,
        });
    ...
    pub fn sync(ptr: *MemoryStorage) !void {
        _ = ptr; // no-op in memory
    }
