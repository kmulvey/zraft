...
test "append entries heartbeat accepted" {
    ...
    try std.testing.expect(node.handleAppendEntries(.{ .term = 2, .leader_id = 2, .prev_log_index = 0, .prev_log_term = 0, .entries = &.{}, .leader_commit = 0 }).success);
}
...
test "append entries rejected on old term" {
    ...
    try std.testing.expect(!node.handleAppendEntries(.{ .term = 3, .leader_id = 2, .prev_log_index = 0, .prev_log_term = 0, .entries = &.{}, .leader_commit = 0 }).success);
}
...
test "follower appends entries from leader" {
    ...
    try std.testing.expect(node.handleAppendEntries(.{ .term = 1, .leader_id = 2, .prev_log_index = 0, .prev_log_term = 0, .entries = &.{wire_entry}, .leader_commit = 0 }).success);
